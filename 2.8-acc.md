...
Eine Zuordnung der Bank-Basiskonten zum Kontenplan sieht bei drei Dimensionen sieht so aus:

![](images/C_AcctSchema_Default-B.PNG)

Die Dimensionen werden durch ``-`` getrennt. Lesart: das Basiskonto ``b_asset_acct`` ist für alle Organisation ``*`` dem Konto ``1200`` zugeordent. Es gibt 83 solcher Kontenkombinationen. Die Kombinationen werden beim [Einrichten eines Mandanten](1.de-mpiere.md) automatisch erstellt.
