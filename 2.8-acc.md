...
Die Dimensionen werden durch ``-`` getrennt. Lesart: das Basiskonto ``b_asset_acct`` ist für alle Organisation ``*`` dem Konto ``1200`` zugeordent. Es gibt 83 solcher Kontenkombinationen. Die Kombinationen werden beim Einrichten eines Mandanten erstellt.
